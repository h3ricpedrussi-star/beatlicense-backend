...
function gerarPDFBuffer(d){
  return new Promise((resolve,reject)=>{
    ...
    let y=110;
    doc.fillColor(C.purple).fontSize(13).font('Helvetica-Bold').text(info.title,ml,y,{align:'center',width:pw});y+=28;
    ...
    doc.moveTo(ml,y).lineTo(ml+pw,y).strokeColor(C.line).lineWidth(0.5).stroke();y+=12;
    function cl(n,t,tx,yp){doc.fillColor(C.purple).fontSize(10).font('Helvetica-Bold').text(n+'. '+t,ml,yp);const ty=yp+16;doc.fillColor(C.dark).fontSize(10).font('Helvetica').text(tx,ml,ty,{width:pw,lineGap:2});return ty+doc.heightOfString(tx,{width:pw,lineGap:2})+12;}
    y=cl('1','OBJETO','O LICENCIANTE concede ao LICENCIADO licenca na modalidade '+info.title+'.',y);
    y=cl('2','LIMITACOES',info.limits,y);
    y=cl('3','DIREITOS AUTORAIS','Direitos morais preservados conforme Lei 9.610/98.',y);
    y=cl('4','CONTRAPRESTACAO','Valor: '+fmtBRL(d.price)+'. Taxa ('+tier.label+'): '+fmtBRL(fee)+'. Repasse: '+fmtBRL(net)+'.',y);
    y=cl('5','VALIDADE','Valida mundialmente por prazo indeterminado.',y);
    if(y>doc.page.height-150){doc.addPage();y=60;}
    y+=10;doc.moveTo(ml,y).lineTo(ml+pw,y).strokeColor(C.line).lineWidth(1).stroke();y+=18;
...
function clicksignAPI(method,path,body){
  ...
  return httpsReq({hostname:'app.clicksign.com',path:path+'?access_token='+CLICKSIGN_TOKEN,method,headers:{'Content-Type':'application/json'}},payload||undefined);
}

async function salvarLicencaDB(d){
  if(!SUPABASE_URL||!SUPABASE_KEY)return null;
  const body=JSON.stringify({produtor_email:d.producerName||'desconhecido',beat_name:d.beatName,buyer_name:d.buyerName,buyer_cpf:d.buyerCpf,buyer_email:d.buyerEmail,license_type:d.licenseType,price:d.price,doc_id:d.docId||null,doc_key:d.docKey||null});
...
app.post('/criar-assinatura',async(req,res)=>{
  ...
  try{
    const r=await stripePost('/v1/checkout/sessions',{'payment_method_types[]':'card','mode':'subscription','line_items[0][price]':STRIPE_PRICE,'line_items[0][quantity]':'1','customer_email':email,'success_url':APP_URL+'/painel.html?assinatura=ativa','cancel_url':APP_URL+'/?cancelado=1','metadata[name]':name||'','locale':'pt-BR'});
    if(r.body.url){res.json({sucesso:true,url:r.body.url,sessionId:r.body.id});}
    else{res.status(500).json({erro:(r.body.error&&r.body.error.message)||'Erro Stripe.'});}
  }catch(err){res.status(500).json({erro:err.message});}
});

app.post('/webhook/stripe',async(req,res)=>{
  const sig=req.headers['stripe-signature'];
  if(STRIPE_WEBHOOK&&sig&&!verifyStripe(req.body.toString(),sig,STRIPE_WEBHOOK)){
    console.error('Webhook Stripe: assinatura invalida');
    return res.status(400).json({erro:'Assinatura invalida.'});
  }
  res.json({ok:true});
  try{
    const evt=JSON.parse(req.body.toString());
    console.log('Stripe evento:',evt.type);
    if(evt.type==='checkout.session.completed'){
      const s=evt.data.object;
      console.log('Assinatura ATIVA:',s.customer_email,s.id);
    ...
    if(evt.type==='customer.subscription.deleted'){
      console.log('Assinatura cancelada:',evt.data.object.customer);
    }
...
app.post('/webhook/clicksign',(req,res)=>{console.log('CS:',JSON.stringify(req.body));res.json({ok:true});});

app.get('/historico/:email',async(req,res)=>{
  if(!SUPABASE_URL||!SUPABASE_KEY)return res.json([]);
  const r=await httpsReq({hostname:new URL(SUPABASE_URL).hostname,path:'/rest/v1/licencas?produtor_email=eq.'+encodeURIComponent(req.params.email)+'&order=created_at.desc&limit=100',method:'GET',headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+SUPABASE_KEY}});
  try{res.json(Array.isArray(r.body)?r.body:[]);}catch(e){res.json([]);}
});

app.get('/ping',(req,res)=>res.json({status:'ok',message:'BeatLicense v7 - Stripe Webhook verificado!'}));
